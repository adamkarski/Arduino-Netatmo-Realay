...
button {
  ...
  <body class="">
    ...
    <div class="container">
      ...
      <footer>
        ...
        <button href="#" class="noselect a_weather" style="position: relative;">
          <div class="weather-temp" style="font-size: 10px; font-weight: bold; position: absolute; top: 2px; width: 100%; text-align: center; color: #fff; text-shadow: 1px 1px 2px black;"></div>
          <img src="https://openweathermap.org/img/wn/04d@2x.png" alt="Prognoza pogody" />
        </button>
    ...
    <div class="ifdata">
      <div class="container-modal" style="display:none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 2000;">
        <div class="modal-back" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5);"></div>

        <div class="weather_widget weather_section flex flex-col items-center justify-center min-h-screen text-gray-700 p-10" style="position: relative; z-index: 2001; pointer-events: none;">
          <!-- Pointer events auto on children to allow clicking -->
          <div class="close noselect bg-white p-10 rounded-xl ring-8 ring-white ring-opacity-40" style="cursor: pointer; pointer-events: auto; margin-bottom: 20px; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; font-size: 30px;">+</div>
          
          <div class="w-full max-w-screen-sm owHidden bg-white p-10 rounded-xl ring-8 ring-white ring-opacity-40" style="pointer-events: auto; max-height: 80vh; overflow-y: auto;">
...
function handleWebSocketMessage(data) {
 ...
 if(parsedData.meta && parsedData.meta.boostEnabled !== undefined){
       ...
       if(boostEnabledInput) boostEnabledInput.checked = (parsedData.meta.boostEnabled === "true");
 }
 if(parsedData.meta && parsedData.meta.boostThreshold !== undefined){
       const boostThresholdInput = document.querySelector("#configModal .boostThreshold");
       if(boostThresholdInput) boostThresholdInput.value = Number(parsedData.meta.boostThreshold);
 }
 if (parsedData.rooms) {
...
document.addEventListener("DOMContentLoaded", function () {
 ...
 const saveBoostThreshold = document.querySelector("#configModal .boostThreshold");
 ...
 if(saveBoostThreshold) {
   saveBoostThreshold.addEventListener("change", function() {
     ws.send(JSON.stringify({ command: "setBoostThreshold", value: this.value }));
   });
 }

 if(boostEnabledCheckbox) {
   boostEnabledCheckbox.addEventListener("change", function() {
     ws.send(JSON.stringify({ command: "setBoostEnabled", value: this.checked }));
   });
 }
